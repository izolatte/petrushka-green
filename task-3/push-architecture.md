...
Система масштабируется горизонтально за счёт очереди и разделения ответственности:
- **Notification Builder** быстро принимает события и формирует задачи на отправку уведомлений
- фактическая отправка выполняется отдельными воркерами (Notification Pull Service / Push Sender), количество которых можно увеличивать при пиковых нагрузках

**Очередь отправки** выступает буфером:
- помогает переживать всплески событий (например, массовые рассылки)
- повышает устойчивость при временной недоступности FCM/APNs (уведомления не теряются, а отправляются позже)

**Event Bus** позволяет подключать новые типы уведомлений без изменения доменных сервисов

При росте нагрузки можно:
- увеличить количество инстансов Notification Pull Service / Push Sender (чтобы ускорить отправку)
- масштабировать **Notification Builder** (чтобы быстрее формировать задачи)
- при необходимости разделить очереди по приоритету (например, сервисные уведомления приоритетнее маркетинговых)
