...
Бизнесовые микросервисы интернет-магазина. Источник событий, от которых могут отправляться пуши.
Примеры событий:
...
**Важно**: сами эти сервисы не должны напрямую отправлять пуш в FCM/APNs, иначе получится сильная связность и будет сложно масштабировать.
...
**Как обычно реализуется:** отдельный микросервис в составе подсистемы уведомлений.
...
То есть Notification Service - это мозг пушей, а отправкой занимается отдельный компонент.
